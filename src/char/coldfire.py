...
class ColdFire(IChar):
    def __init__(self, skill_hotkeys, char_config, screen: Screen, template_finder: TemplateFinder, ui_manager: UiManager, pather: Pather):
        Logger.info("Firing up Cold/Fire Sorceress")
    ...
    def _main_attack_cold(self, cast_pos_abs: Tuple[float, float], delay: float, spray: float = 10):
        keyboard.send(self._skill_hotkeys["skill_right_cold"])
        x = cast_pos_abs[0] + (random.random() * 2*spray - spray)
        y = cast_pos_abs[1] + (random.random() * 2*spray - spray)
        cast_pos_monitor = self._screen.convert_abs_to_monitor((x, y))
        mouse.move(*cast_pos_monitor)
    ...
    def _main_attack_fire(self, cast_pos_abs: Tuple[float, float], delay: float, spray: float = 10):
        keyboard.send(self._skill_hotkeys["skill_right_fire"])
        r = (random.random() * 2*spray - spray)*10
        x = cast_pos_abs[0] + r
        y = cast_pos_abs[1]
        cast_pos_monitor = self._screen.convert_abs_to_monitor((x, y))
        mouse.move(*cast_pos_monitor)
    ...
    def kill_pindle(self) -> bool:
        ...
        if pindle_pos_abs is not None:
            ...
            for _ in range(int(self._char_config["atk_len_pindle"])):
                self._main_attack_cold(cast_pos_abs, delay, 15)
                self._left_attack(cast_pos_abs, delay, 15)
            wait(0.1, 0.15)
            self._main_attack_fire(cast_center, delay, 30)
            # Move to items
            if self._config.char["static_path_pindle"]:
    ...
    def kill_eldritch(self) -> bool:
        ...
        if pos_abs is not None:
            eld_pos_abs = [pos_abs[0] * 1.1, pos_abs[1] * 1.1]
        else:
            eld_pos_abs = self._screen.convert_screen_to_abs(self._config.path["eldritch_end"][0])
        if eld_pos_abs is not None:
            cast_pos_abs = [eld_pos_abs[0], eld_pos_abs[1]]
            for _ in range(int(self._char_config["atk_len_eldritch"])):
                self._main_attack_fire(cast_center, delay, 40)
                self._left_attack(cast_pos_abs, delay, 100)
            wait(0.2, 0.3)
            # Move to items
    ...
    def kill_shenk(self) -> bool:
        ...
        if shenk_pos_abs is not None:
            ...
            for _ in range(int(self._char_config["atk_len_shenk"])):
                self._main_attack_fire(cast_pos_abs, delay, 90)
...
if __name__ == "__main__":
    ...
    char = ColdFire(config.coldfire, config.char, screen, t_finder, ui_manager, pather)
    # char.pre_buff()
    # char.tp_town()
